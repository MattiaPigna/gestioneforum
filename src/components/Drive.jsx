...
import { FileText, Sheet, Folder, Presentation, ExternalLink, Search, Filter, Plus, X, Loader2, Trash2, Link } from 'lucide-react';
    ...
    <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Es. Verbale riunione.pdf" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} />
              </div>
              <div>
                ...
                <div className="flex items-center gap-2 border border-slate-200 rounded-xl px-3 focus-within:ring-2 focus-within:ring-blue-400">
                  ...
                  <input className="flex-1 py-2 text-sm focus:outline-none" placeholder="https://drive.google.com/file/d/..." value={form.link} onChange={e => setForm(f => ({ ...f, link: e.target.value }))} />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Es. 2.3 MB" value={form.dimensione} onChange={e => setForm(f => ({ ...f, dimensione: e.target.value }))} />
                </div>
              </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1 flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-4 py-2.5 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
          ...
          <input className="flex-1 text-sm bg-transparent focus:outline-none text-slate-700 placeholder-slate-400" placeholder="Cerca file..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
          ...
          return (
            <div key={file.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group p-4">
              <div className="flex items-start gap-3">
                ...
                <button onClick={() => handleDelete(file.id)} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-rose-500 transition-all shrink-0">
                  <Trash2 size={14} />
                </button>
              </div>
