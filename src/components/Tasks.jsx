...
import { Plus, X, CheckCircle, Circle, Clock, AlertCircle, Trash2, Loader2, Download, FileText } from 'lucide-react';
  ...
  return (
    <div className={`bg-white rounded-xl p-4 shadow-sm border border-slate-100 border-l-4 ${p.border} hover:shadow-md transition-all group`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <button onClick={() => onDelete(task.id)} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-rose-500 transition-all shrink-0">
          <Trash2 size={14} />
        </button>
      </div>
      <p className="text-xs text-slate-500 mt-1.5 line-clamp-2">{task.descrizione}</p>
...
export default function Tasks() {
  ...
  const [form, setForm] = useState({ titolo: '', descrizione: '', priorita: 'Media', stato: 'To Do', assegnatario: '', scadenza: '' });
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-slate-800">Nuovo Task</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Nome cognome..." value={form.assegnatario} onChange={e => setForm(f => ({ ...f, assegnatario: e.target.value }))} />
              </div>
              <div>
          ...
          return (
            <div key={col} className={`rounded-2xl border ${cfg.headerBorder} overflow-hidden`}>
              ...
              <div className="p-3 space-y-3 min-h-32 bg-slate-50/50">
                {colTasks.map(task => <TaskCard key={task.id} task={task} onDelete={handleDelete} />)}
                {colTasks.length === 0 && (
